...
agents over the fabric) and is CI-testable in-process — **done**, see `fabric::Pipeline`
/ `Stage` and §7 M4c. Hierarchy/Market/Swarm build on the same `Stage` + fabric
routing.
...
| **M4c** ⏳ | **teams**: `Team` execution for the Pipeline paradigm first, then Hierarchy/Market/Swarm. **Pipeline done** — `fabric::Pipeline` threads a `VectorMessage` through member agents in order; each member is a `Stage` (its model forward-pass, a `MapStage` transform today). **Every hop is a real capability-gated fabric `send`** so INV-2 is enforced *between* members, and **INV-3 is checked on entry to each stage** (a cross-`ModelFingerprint` aligned payload must be marked `Translate`, never implicitly injected). Tested: end-to-end 3-stage transform, a mis-scoped hop capability is denied, and a cross-fingerprint hop is rejected unless translated. Hierarchy/Market/Swarm next. | ✅ in-process (CI) |
