...
- **F14 Agent learning**: agents distill **skills** from their own audit traces (the INV-4 ledger as each agent's experience), adopt them under falsification gates, and diffuse them to peers over the fabric — know-how moves freely, authority never does (skill ≠ capability) ([RFC-0009](rfcs/0009-learning-and-self-evolution.md)).
- **F15 Self-evolution & portability**: distinct from self-healing (F3 restores; F15 *improves*) — an agent senses its own inadequacy and bids, from its own attention budget, for a stronger mind (model rebind), a fitter body (migration), or more attention; and can be exported as a **sealed offline bundle** and re-admitted into a foreign trust domain with its competence intact and its old authority void ([RFC-0009](rfcs/0009-learning-and-self-evolution.md)).
...
| | M9 learning & self-evolution (**parallel to M6**; numbered by allocation, not execution order) | skill distillation/diffusion + substrate seeking (bid market over INV-1) + sealed portable bundles (air-gap deploy, trust re-rooting) | delivers F14/F15 and the first concrete F9 leg; the bottom-up dual of M5's top-down governance — **design drafted ([RFC-0009](rfcs/0009-learning-and-self-evolution.md)): zero new TAM operations, all four stages CI-able, gates E10–E13** |
| **H2 Specialization (riding Linux)** | M6 push the stack down | eBPF observability/security → unikernel/abstract-machine contract → kernel-bypass vector transport → FPGA primitives | a real efficiency curve → Series B/strategic investment — **design drafted ([RFC-0008](rfcs/0008-push-the-stack-down.md)): meter → replace → verify; the substrate ledger (eBPF attribution joined to INV-4, + INV-2 compiled to kernel deny-floors) is the meter every later stage is judged by; the contract guest (guest-runner as PID 1, vmproto-only surface), the bypass vector data plane (shm ring → io_uring → AF_XDP; medium-agnostic ring contract with a CXL memory-semantic rung on the horizon), and an FPGA capability-verify core — each behind its own falsification gate (E6–E9), and any round Linux wins, Linux keeps** |
| **H3 Co-designed silicon** | M7 single-primitive tape-out | tape out just one primitive that is uniquely yours (vector-transport NIC / capability memory controller — nearest concrete form: a CXL Type-3 memory device enforcing INV-2 in the controller, riding the commodity CXL fabric instead of a bespoke interconnect ([RFC-0008 §8](rfcs/0008-push-the-stack-down.md)) / dataflow attention engine) | a hardware moat → large hardware financing |
